...
export const FenceDimension: React.FC<FenceDimensionProps> = ({ sides, gate, description }) => {
    return (
        <View className="items-start px-2 pt-3 pb-14 rounded-3xl border border-solid border-stone-300">
            <View className="w-full">
                ...
                {sides.map((side) => (
                    <Side key={side.sideNumber} sideNumber={side.sideNumber} sideInfo={side.sideInfo} />
                ))}
                ...
                <View className="items-center px-2 py-1.5 w-full text-xs font-semibold">
                    <View className="w-full">
                        <View>
                            <Text className="font-semibold text-emerald-950">Description</Text>
                        </View>
