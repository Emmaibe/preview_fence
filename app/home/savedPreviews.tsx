...
const SavedPreviews = ({ asComp }: { asComp: boolean }) => {
  const { savedFences, setSelectedFence, setSelectedPreview } =
    usePreviewContext();

  const [search, setSearch] = useState("");

  const { previewLoading, previewError, previews, fetchSavedPreviews } =
    usePreviews();
    ...
    <SafeAreaView className="p-4 flex-1">
      {!asComp && (
        <TouchableOpacity
          ...
          className="flex flex-row items-center space-x-2 mb-8"
        >
          <Image
      ...
      {previews.length > 0 && (
        <View>
          ...
          <View className="flex-row items-center mt-4 w-full justify-between">
            ...
            <Text className="font-intermedium text-sm text-gray-400">
              Previews
            </Text>
            <View className="border-t-2 border-gray-100 w-[42%]"></View>
          </View>

          <Text className="text-primary-text text-base font-intersb mt-2 mb-3">
            {previews.length} saved previews found
          </Text>
        </View>
      )}
        ...
        <View className="h-full justify-center items-center bottom-8">
          ...
          >
            ...
            <View className="space-y-2">
              <Text className="text-center font-intersb text-[16px]">
                No saved previews
              </Text>
